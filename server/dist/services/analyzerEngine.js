...
const resolveVictimProfileFromTransfers = (transfers, preferredAccount) => {
    if (!preferredAccount)
        return null;
    const matched = transfers.find((transfer) => accountMatches(transfer.senderAccount, preferredAccount) ||
...
const derivePrimaryVictim = (bundle, preferredAccount) => {
    ...
    for (const transfer of transfers) {
        ...
        sentAmount.set(transfer.senderAccount, (sentAmount.get(transfer.senderAccount) ?? 0) + transfer.amount);
    ...
    const candidates = [...new Set(transfers.map((item) => item.senderAccount))].sort((left, right) => {
        ...
        if (leftInbound === 0 && rightInbound !== 0)
            return -1;
        if (rightInbound === 0 && leftInbound !== 0)
            return 1;
...
const accountCandidates = (value) => {
    ...
    if (!raw)
        return [];
    const candidates = new Set([raw, digitsOnly(raw)]);
...
const getBundleStartTimestamp = (bundle) => {
    const timestamps = [
...
const deriveExposureAmount = (bundle) => {
...
const buildMoneyTrail = (bundle, preferredVictimAccount) => {
    const transfers = bundle.transfers;
    const participantProfiles = Object.fromEntries(transfers.flatMap((item) => [
        [
            item.senderAccount,
            {
    ...
    if (!transfers.length) {
        ...
        const bankNodes = [
            ...new Set([...bundle.withdrawals.map((item) => item.bankName), ...bundle.bankActions.map((item) => item.bankName)].filter((item) => Boolean(item)))
        ].map((bankName) => ({
            id: `bank:${bankName}`,
    ...
    const path = [];
    const root = resolveVictimProfileFromTransfers(transfers, preferredVictimAccount)?.accountNumber ??
    ...
    return {
        ...
        edges,
        path,
        layerDistribution: Object.fromEntries(transfers.reduce((acc, item) => {
        ...
        flags: {
            ...
            circularTransfers: transfers.some((item) => transfers.some((candidate) => candidate.senderAccount === item.receiverAccount &&
...
const buildWithdrawalIntelligence = (withdrawals) => {
    const totalWithdrawn = withdrawals.reduce((sum, item) => sum + item.amount, 0);
    const distribution = Object.fromEntries(withdrawals.reduce((acc, item) => {
...
const buildRecovery = (bundle, totalAmount) => {
...
const buildPatterns = (bundle) => {
    ...
    return {
        ...
        repeatedIfsc: [...new Set(repeatedIfsc.filter((item) => Boolean(item)))],
        highFrequencySmallTransactions: bundle.smallTransactions.length,
        muleAccounts: [
            ...new Set(bundle.withdrawals
                .map((item) => item.accountNumber)
                .filter((item) => Boolean(item)))
        ]
    };
};
const buildRisk = (bundle, totalAmount, timeline, withdrawal) => {
...
const buildBankInsights = (bundle, recovery) => {
    const banks = new Map();
    [...bundle.transfers, ...bundle.withdrawals].forEach((item) => {
    ...
    bundle.bankActions.forEach((action) => {
        if (!action.bankName || !action.timestamp || !baseline)
            return;
        const bank = banks.get(action.bankName) ?? { name: action.bankName, freezeCount: 0, responseTimeHours: [] };
        bank.responseTimeHours.push(Math.max((action.timestamp.getTime() - baseline.getTime()) / 36e5, 0));
    ...
    return [...banks.values()].map((bank) => ({
        ...
        avgResponseTime: bank.responseTimeHours.length > 0
            ? bank.responseTimeHours.reduce((sum, value) => sum + value, 0) / bank.responseTimeHours.length
            : 0
    }));
};
const deriveBundleMap = (parsed) => {
    const map = new Map();
    const ensure = (acknowledgementNo) => {
...
const calculateBundleAmount = (bundle) => deriveExposureAmount(bundle);
const calculateBundleEvidenceScore = (bundle) => bundle.transfers.length * 4 +
    bundle.withdrawals.length * 3 +
    bundle.holds.length * 2 +
    bundle.bankActions.length * 2 +
    bundle.smallTransactions.length;
const selectBundleForExistingCase = (caseRecord, bundleMap) => {
    const bundles = [...bundleMap.values()];
    if (!bundles.length) {
        return null;
    }
    const exactComplaintMatch = bundles.find((bundle) => bundle.acknowledgementNo === caseRecord.complaintId);
    if (exactComplaintMatch) {
        return exactComplaintMatch;
    }
    if (isReliableAccountInput(caseRecord.victimAccount)) {
        const victimAccountMatch = bundles.find((bundle) => bundle.transfers.some((transfer) => accountMatches(transfer.senderAccount, caseRecord.victimAccount) ||
    ...
    if (normalizedVictimName) {
        const victimNameMatch = bundles.find((bundle) => bundle.transfers.some((transfer) => transfer.victimName?.trim().toLowerCase() === normalizedVictimName));
        if (victimNameMatch) {
    ...
    if (normalizedVictimMobile) {
        const victimMobileMatch = bundles.find((bundle) => bundle.transfers.some((transfer) => transfer.victimMobile?.trim().toLowerCase() === normalizedVictimMobile));
    ...
    if (caseRecord.fraudAmount > 0 && (!isReliableAccountInput(caseRecord.victimAccount) || !hasEnoughDigits(caseRecord.complaintId))) {
        return bundles
            .slice()
            .sort((left, right) => {
            const amountDelta = Math.abs(calculateBundleAmount(left) - caseRecord.fraudAmount) -
    ...
    return bundles
        ...
        return (Math.abs(calculateBundleAmount(left) - caseRecord.fraudAmount) -
            Math.abs(calculateBundleAmount(right) - caseRecord.fraudAmount));
    })[0];
};
const buildDatasetDiagnostics = (bundleMap) => {
    const bundles = [...bundleMap.values()];
    ...
    if (acknowledgements.length > 1) {
        warnings.push(`Workbook contains ${acknowledgements.length} distinct acknowledgement groups; analysis should prioritize the dominant evidence bundle.`);
    }
    if (dominant && dominant.transferCount === 0) {
        warnings.push(`Dominant acknowledgement ${dominant.acknowledgementNo} has no money-trail transfer rows; timeline and withdrawal intelligence are available, but transfer graph is incomplete.`);
    }
    return {
...
const upsertBank = async (name, ifsc) => {
    if (!name)
        return null;
...
const syncBundleToDatabase = async (caseId, bundle) => {
    ...
    for (const action of bundle.bankActions) {
        ...
        const responseTimeHours = action.timestamp && bundle.transfers[0]?.timestamp
...
const ensureCase = async (bundle, officerId) => {
    const primaryVictim = derivePrimaryVictim(bundle);
    const totalAmount = deriveExposureAmount(bundle);
    const fraudTimestamp = bundle.transfers.map((item) => item.timestamp).find(Boolean) ??
...
const upsertCaseAnalysisRecord = async (caseId, bundle, preferredVictimAccount) => {
    const totalAmount = deriveExposureAmount(bundle);
...
export const ingestAnalyzerDatasetForCase = async (filePath, caseRecord) => {
...
const normalizeWorkbookFileName = (filePath) => {
    const rawFileName = filePath.split("/").pop() ?? filePath;
    return rawFileName.replace(/^\d+-/, "");
};
const normalizeSheetNameForReport = (sheetName) => {
    const compact = sheetName.trim().toLowerCase();
    if (compact.includes("money transfer"))
        return "Money Transfer to";
    if (compact.includes("others less then 500"))
        return "Others Less Then 500";
    if (compact.includes("transaction put on hold"))
        return "Transaction put on hold";
    if (compact.includes("withdrawal through atm"))
        return "Withdrawal through ATM";
    if (compact.includes("cash withdrawal through cheque"))
        return "Cash Withdrawal through Cheque";
    if (compact.includes("withdrawal through pos"))
        return "Withdrawal through POS";
    if (compact === "other")
        return "Other";
    if (compact.includes("aeps"))
        return "AEPS";
    if (compact.includes("customer se"))
        return "Withdrawal through Customer Service";
    if (compact.includes("funds not received by cc"))
        return "Funds Not Received by CC";
    return sheetName.trim();
};
const buildBundleAnalysis = (bundle) => {
    const totalAmount = calculateBundleAmount(bundle);
...
const buildAnalyzerReport = (filePath, parsed, diagnostics, dominantCase) => {
    if (!dominantCase) {
    ...
    const distribution = Object.fromEntries(Object.entries(dominantCase.withdrawalIntelligence.withdrawalTypeDistribution).map(([channel, amount]) => [
        channel,
        {
    ...
    const multiChannelWithdrawal = Object.keys(dominantCase.withdrawalIntelligence.withdrawalTypeDistribution).length >= 3;
    const chequeDominance = Number(dominantCase.withdrawalIntelligence.withdrawalTypeDistribution.CHEQUE ?? 0) >
        totalWithdrawn * 0.5;
    const warnings = [
    ...
    return {
        ...
        timeline: {
            ...
            interpretation: dominantCase.timeline.delayCategory === "CRITICAL"
                ? [
        ...
        microFraudAnalysis: {
            ...
            insights: dominantCase.smallTransactionCount > 100
                ? [
        ...
        finalVerdict: {
            ...
            recoveryStatus: dominantCase.recovery.recoveryRate >= 0.7 ? "STRONG" : dominantCase.recovery.recoveryRate >= 0.3 ? "PARTIAL" : "LOW",
            investigationPriority: dominantCase.risk.level === "CRITICAL" ? "HIGH" : "MEDIUM"
        }
    };
};
export const analyzeWorkbookFile = (filePath) => {
...
export const ingestAnalyzerDataset = async (filePath, officerId) => {
    const parsed = parseAnalyzerWorkbook(filePath);
    const bundleMap = deriveBundleMap(parsed);
    const results = [];
...
export const getAnalyzerSummary = async () => {
    ...
    const recoveredAmount = analyses.reduce((sum, item) => sum + Number(item.recovery?.recoveredAmount ?? 0), 0);
    return {
        totalCases,
        totalAmount,
        recoveredAmount,
        criticalCases: analyses.filter((item) => item.risk?.level === "CRITICAL").length,
...
export const getBankPerformanceAnalysis = async () => {
    ...
    analyses.forEach((analysis) => {
        const bankInsights = analysis.bankInsights ?? [];
        bankInsights.forEach((insight) => {
            const current = byBank.get(insight.bankName) ?? {
    ...
    const ranked = [...byBank.values()].map((bank) => ({
        ...
        freezeCount: bank.freezeCount,
        recoveryRate: bank.recoveryRate.reduce((sum, value) => sum + value, 0) / Math.max(bank.recoveryRate.length, 1),
        avgResponseTime: bank.response.reduce((sum, value) => sum + value, 0) / Math.max(bank.response.length, 1)
    }));
    return {
...
export const getCaseAnalysisById = async (caseId) => prisma.caseAnalysis.findUnique({
    where: { caseId },
    include: { case: true }
});
